...
def demo_object(data_idx=11, object_idx=0):
    ...
    obj.print_object()
    # utils.Calibration(calib_filename)
...
def demo(data_idx=11, object_idx=0, show_images=True, show_lidar=True, show_lidar_2d=True, show_lidar_box=True, show_project=True, show_lidar_frustum=True):
    ...
    if show_lidar_2d:
        ...
        show_lidar_on_image(pc_velo, img, calib, img_width,
                            img_height, showtime=True)
        raw_input()
...
def extract_frustum_data(idx_filename, split, output_filename, viz=False,
    ...
    for data_idx in tqdm(data_idx_list):
        ...
        if with_image:
            # dataset.get_image(data_idx)#(370, 1224, 3),uint8
            image_filename = os.path.join(
                dataset.image_dir, '%06d.png' % (data_idx))
        objects = dataset.get_label_objects(data_idx)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--show_stats', action='store_true', help='show_stats.')

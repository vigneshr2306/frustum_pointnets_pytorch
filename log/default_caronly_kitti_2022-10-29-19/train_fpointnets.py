...
parser.add_argument('--model', default='frustum_pointnets_v1_old',
...
parser.add_argument('--decay_step', type=int, default=20,
                    help='Decay step for lr decay [default: 60]')
...
def train():
    ...
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer=optimizer, lr_lambda=lr_func)
    ...
    for epoch in range(MAX_EPOCH):
        ...
        log_string('box IoU(ground/3D): %.6f/%.6f' %
                   (train_iou2d, train_iou3d))
        log_string('box estimation accuracy (IoU=0.7): %.6f' %
                   (train_iou3d_acc))
        ...
        log_string('%s box IoU(ground/3D): %.6f/%.6f' %
                   (blue('test'), test_iou2d, test_iou3d))
        log_string('%s box estimation accuracy (IoU=0.7): %.6f' %
                   (blue('test'), test_iou3d_acc))
        log_string("learning rate: {:.6f}".format(
        ...
        if FLAGS.return_all_loss and not FLAGS.debug:
            ...
            writer.add_scalar('train_size_class_loss',
                              train_size_class_loss, epoch)
        ...
        if FLAGS.return_all_loss:
            ...
            writer.add_scalar('test_size_class_loss',
                              test_size_class_loss, epoch)
        ...
        log_string('Best Test acc: %f(Epoch %d)' %
                   (best_iou3d_acc, best_epoch))

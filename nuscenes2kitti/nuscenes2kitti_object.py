...
class nuscenes2kitti_object(object):
    ...
    def __init__(self, root_dir, split='v1.0-mini', sensor_list=['CAM_FRONT']):
        ...
        self.CAM_FRONT_RIGHT_dir = os.path.join(
            self.split_dir, 'image_CAM_FRONT_RIGHT')
        self.CAM_BACK_RIGHT_dir = os.path.join(
            self.split_dir, 'image_CAM_BACK_RIGHT')
        self.CAM_BACK_dir = os.path.join(self.split_dir, 'image_CAM_BACK')
        self.CAM_BACK_LEFT_dir = os.path.join(
            self.split_dir, 'image_CAM_BACK_LEFT')
        self.CAM_FRONT_LEFT_dir = os.path.join(
            self.split_dir, 'image_CAM_FRONT_LEFT')

        self.calib_dir = os.path.join(self.split_dir, 'calib')
        self.lidar_dir = os.path.join(self.split_dir, 'LIDAR_TOP')

        self.label_CAM_FRONT_dir = os.path.join(
            self.split_dir, 'label_CAM_FRONT')
        self.label_CAM_FRONT_RIGHT_dir = os.path.join(
            self.split_dir, 'label_CAM_FRONT_RIGHT')
        self.label_CAM_BACK_RIGHT_dir = os.path.join(
            self.split_dir, 'label_CAM_BACK_RIGHT')
        self.label_CAM_BACK_dir = os.path.join(
            self.split_dir, 'label_CAM_BACK')
        self.label_CAM_BACK_LEFT_dir = os.path.join(
            self.split_dir, 'label_CAM_BACK_LEFT')
        self.label_CAM_FRONT_LEFT_dir = os.path.join(
            self.split_dir, 'label_CAM_FRONT_LEFT')
    ...
    def get_image(self, sensor, idx):
        ...
        img_filename = os.path.join(
            getattr(self, sensor+'_dir'), '%06d.jpg' % (idx))
    ...
    def get_label_objects(self, sensor, idx):
        ...
        label_filename = os.path.join(
            getattr(self, 'label_'+sensor+'_dir'), '%06d.txt' % (idx))
    ...
    for obj in objects:
        ...
        for i in range(4):
            cv2.line(img,
                     ...
                     (int(corners_2d.T[i + 4][0]),
                      int(corners_2d.T[i + 4][1])),
...
def show_image_with_boxes(img, objects, calib, sensor, show3d=True, linewidth=2, colors=((0, 0, 255), (255, 0, 0), (155, 155, 155))):
    ...
    for obj in objects:
        if obj.type == 'DontCare':
            continue
        if obj.type not in type2color.keys():
            continue
...
def project_velo_to_image(pc_velo, calib, sensor, return_time=False):
    ...
    pts_3d_global = translate(pts_3d_global, getattr(
        calib, 'ego2global_translation'))

    # Third step: transform into the ego vehicle frame for the timestamp of the image.
    pts_3d_ego_cam = translate(
        pts_3d_global, -getattr(calib, sensor+'_'+'ego2global_translation'))
    pts_3d_ego_cam = rotate(pts_3d_ego_cam, getattr(
        calib, sensor+'_'+'ego2global_rotation').T)

    # Fourth step: transform into the camera.
    pts_3d_cam = translate(
        pts_3d_ego_cam, -getattr(calib, sensor+'_'+'cam2ego_translation'))
    pts_3d_cam = rotate(pts_3d_cam, getattr(
        calib, sensor+'_'+'cam2ego_rotation').T)

    # Take the actual picture (matrix multiplication with camera-matrix + renormalization).
    depths = pts_3d_cam[2, :]
    pts_2d_cam = utils.view_points(pts_3d_cam[:3, :], getattr(
        calib, sensor), normalize=True)  # (3,n)
...
def get_lidar_in_image_fov(pc_velo, calib, sensor, xmin, ymin, xmax, ymax,
    ...
    depths = pts_3d_cam[2, :]
    pts_2d = utils.view_points(pts_3d_cam[:3, :], getattr(
        calib, sensor), normalize=True)  # (3,n)
    pts_2d = pts_2d.T
    pts_2d[:, 2] = depths.T
    ...
    # (3067, 3),mean:array([-1.1616094e-02,  1.8041308e-01,  1.5962053e+01], dtype=float32)
...
def dataset_viz():
    dataset = nuscenes2kitti_object(os.path.join(
    ...
    for data_idx in range(len(dataset)):
        ...
        show_lidar_with_boxes(pc_velo, objects, calib,
                              sensor, True, img_width, img_height)
        raw_input()
